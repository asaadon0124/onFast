														...
														<td>
															@if ($product->status_id == 1)
																<div class="btn-icon-list">
																	...
																	<a href="" class="makeAproveProduct" product_id="{{ $product->id }}">
																		<button class="btn btn-primary btn-icon">موافقة</button>
																	</a>&nbsp;

																	<a href="" class="makeDeleteProduct" product_id="{{ $product->id }}">
																		<button class="btn btn-danger btn-icon">حزف</button>
																	</a>&nbsp;

                                                                    <a href="{{ route('products.newProducts_edit',$product->id) }}">
																		<button class="btn btn-indigo btn-icon">تعديل</button>
																	</a>
																</div>
															@endif
														</td>
...
{{--  APROVE PRODUCT   --}}
	<script>
		$(document).on('click','.makeAproveProduct',function(e)
		{
			e.preventDefault();
			...
			{
				type: 'post',
				url: "{{route('products.newProducts_accept')}}",
				data:
				{
					'_token' : "{{ csrf_token() }}",
					'id'     : product_id
