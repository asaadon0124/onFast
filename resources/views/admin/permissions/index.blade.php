...
@section('content')
				...
				{{--  END GET FLASH MESSAGES   --}}
								...
								<div class="table-responsive">
									<table class="table text-md-nowrap" id="example1" rowID="{{ $permissions->count()+1 }}">
										<thead>
											<tr>
												<th class="wd-15p border-bottom-0"> رقم</th>
												<th class="wd-15p border-bottom-0">اسم الصلاحية</th>
										...
										<tbody>
											...
											@foreach ($permissions as $permission)
												<tr class="adminRow{{ $permission->id }}">
													<td>{{ $x++ }}</td>
													<td>{{ $permission->name }}</td>
													<td>
													<div class="btn-icon-list">
														<a href="{{ route('permissions.edit',$permission->id) }}" style="margin-left: 10px">
															<button class="btn btn-success btn-icon"><i class="typcn typcn-document-add"></i></button>
														</a>
														<a href="" class="makeDeleteAdmin" admin_id="{{ $permission->id }}">
											...
											@endforeach
											@else
												<h1 class="text-center">لا يوجد صلاحيات</h1>
											@endif
										</tbody>
									</table>
								</div>
