...
class productsController extends Controller
{
    public function index()
    {
        ...
        {
            $products           = Product::where('supplier_id',$supplier->id)->where('status_id',1)->whereDoesntHave('orders_detailes')->whereBetween('created_at',array('2024-11-01', Carbon::now()))->orderByDesc('id')->get();
            $productCompleted   = OrderDetailes::with('product2','order2')->where('order_id','<>',null)->whereHas('product2',function($q) use($supplier)
            {
            ...
            return view('user.products.index',compact('productCompleted','products'));
        }
    }
