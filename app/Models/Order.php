...
class Order extends Model
{
    ...
    {
        return $this->hasMany(Returns::class,'order_id','id')->withTrashed();
    }
    
    
     public function scopeSearch($query, $value)
    {
        return $query->whereHas('servant', function ($q) use ($value) {
            $q->where('name', 'like', '%' . $value . '%');
        })->orWhereHas('orders_detailes.product.cities', function ($q) use ($value) {
            $q->where('name', 'like', '%' . $value . '%');
